...
```{r sl_feature_sel}
# We use the existing screener for randomForest here out of expedience.
library(randomForest)

# Existing screening wrappers
SuperLearner::listWrappers(what = "screen")

# View the code - it's pretty short.
# By default it's picking the top 10 variables based on RF importance.
screen.randomForest
...
cv_sl =
  SuperLearner::CV.SuperLearner(Y = df[[outcome]],
                                ...
                                X = df[, setdiff(predictors, c("degree", "city"))],
                                verbose = FALSE,
                                # Quick feature selection example
                                 SL.library = list("SL.mean", "SL.glmnet", c("SL.ranger", "screen.randomForest")),
                                family = binomial(),
                                # For a publication we would do V = 10 or 20
                                cvControl = list(V = 2L, stratifyCV = TRUE))
summary(cv_sl)
...
Hoffman, K. (2020). Become a Superlearner! An Illustrated Guide to Superlearning. https://www.khstats.com/blog/sl/superlearning
